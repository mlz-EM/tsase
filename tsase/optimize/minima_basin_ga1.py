...
##GZG
    def check_pop(self, cons):
        """Returns pop without some members based on history_num"""
        # copied from runnning_loop ~900
    ...
    def get_fitness(self, cons):
        """Return fitness value for the nearest local minimum"""
        #np.set_printoptions(threshold=np.inf)
        if self.fit_func == 'energy':
            fitness = np.zeros(len(cons))
            energy = np.zeros(len(cons))
            for i in range(len(cons)):
                energy[i] = self.get_energy(self.atoms_GA[i].get_positions())
...
##GZG
            ...
            if self.move_type == 2:
                self.get_fitness(self.atoms_GA)
            ...
            if accept:
                ...
                if self.history_num:
                    ...
                    #print('oe',oldEnergy)#print(self.temp_minima[int(self.num_accepted_moves)])
                    ...
                    #print('geo1',self.geometries) # minimum energy: integer name
                    #print('geoh1',self.geo_history) # integer name: number of times landed on
                    ...
                    if self.use_geo and self.num_accepted_moves >= self.history_num:
                        ...
                        #print('geo',self.geometries) # minimum energy: integer name
                        #print('geoh',self.geo_history) # integer name: number of times landed on
                        i = 0
