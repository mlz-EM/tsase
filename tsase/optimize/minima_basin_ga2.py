    ...
    def get_fitness(self, cons):
        ...
        if self.fit_func == 'energy':
            ...
            for i in range(len(cons)):
                ...
                #print('poopoo',cons[i])
                energy[i] = self.get_energy(cons[i])
                #print('wtf',energy[i])
            print('e',energy,np.amin(energy))
            ##TODO fix divide by zero error for energymax = energymin
...
##GZG
    ...
    def accept_criteria_GA(self, fitness):
        ...
        if self.fit_accept_criteria == 'deterministic':
            for i in range(len(fitness)):
                x = 60
                if fitness[i] >= np.percentile(fitness, x): # accepts structures in x percentile of fitness
                    accept_GA[i] = 1
                else:
                    accept_GA[i] = 0
        #print('aGA',accept_GA)
    ...
    def reproduction(self, cons, accept_GA):
        reppop = np.zeros(len(cons),dtype=object)
        accept_count = 0
        for i in range(len(cons)):
            if accept_GA[i] == 1:
                #print(cons[i])
                reppop[accept_count] = cons[i]
                accept_count += 1
        print(reppop)
...
#            tsase.io.write_con('movie_MHBH.con',self.atoms,w='w')
        ...
        for i in range(gens): 
            print('gen',i)
            for j in range(len(self.atoms_GA)):
                print('member',j)
                self.positions = self.atoms_GA[j].get_positions()
            ...
            print('aGA',accept_GA)
            self.reproduction(cons,accept_GA)
        #print(cons)
        #fitness = self.get_fitness(cons)
        #print(fitness)
...
#                    self.positions = self.atoms_GA[j].get_positions()
#                    self.atoms.set_positions(self.positions)
#                    self.running_loop_GA(self.positions, steps)
#                    cons[j] = self.positions
#                #print('c',cons)
#                fitness = self.get_fitness(cons)
#                print('fit1',fitness)
