...
##GZG                 
                 ...
                 dyn_fert_weight = 1.0,
                 accept_style = 'choose_parents_first',
                 elitism = True,
...
##GZG        
        ...
        self.accept_style = accept_style
...
##GZG this seems to be broken, maybe not now?
    ...
    def get_energies(self, cons):
        '''returns array of energies and approximate energies
        of local minima for array of atom objects, and updates history'''
...
#    def get_energies_db(self, aGA):
#        energies = np.zeros(aGA.get_number_of_unrelaxed_candidates())
#        print('e',energies)
    ...
    def push_apart(self,positions):
        """ Push atoms' positions apart until all atoms are no 
        closer than self.pushapart distance from one another."""
...
##GZG
    ...
    def accept_criteria_GA(self, fitness):
        ...
        if self.accept_style == 'choose_parents_first':
    ...
    def crossover(self, parent0, parent1):
        half0 = np.sort(parent0,0)[:int(np.ceil(len(parent0)/2))]
        half1 = np.sort(parent1,0)[int(np.ceil(len(parent1)/2)):]
        twohalves = np.append(half0, half1, axis = 0)
        result = self.push_apart(twohalves)
        return(result)

    def remove_trailing_integers(self, sequence):
        while type(sequence[-1]) == int:
        ...
        if self.accept_style == 'choose_parents_first':
            ...
            for i, con in enumerate(cons):
                if accept_GA[i] != 0:
                    ...
                    accept_count += 1
            reppop = self.remove_trailing_integers(sequence = reppop)
            fits = self.remove_trailing_integers(sequence = fits)
            ...
            for i, Reppop in enumerate(reppop):
                for j in range(int(childs[i])):
                    ...
                    newpop[k] = self.move(step = 1, ro = self.positions, 
            ...
            if self.favor == 'random':
                ...
                for i in range(leftover):
                    ...
                    self.atoms.set_positions(self.positions)
                    newpop[-(i+1)] = self.move(step = 1, ro = self.positions, 
                                               distribution = self.distribution)

        elif self.accept_style == 'roulette':
            exp_scale = 3
            accept_count = 0
            runpop = 0
            while type(newpop[-1]) == int:
                print('run',runpop)
                runpop += 1
                if self.fit_accept_criteria == 'exponential':
                    for i, con in enumerate(cons):
                        truefit = np.exp(-fitness[i]*exp_scale)
                        if truefit < np.random.uniform(0,1) and type(newpop[-1]) == int:
                            self.positions = con
                            self.atoms.set_positions(self.positions)
#                            print(con)
#                            print('ac',accept_count)
                            #print('com',self.atoms.get_center_of_mass())
                            print('con',self.Ens[i])
                            newpop[accept_count] = self.move(step = 1, ro = self.positions, 
                                                             distribution = self.distribution)
                            accept_count += 1
...
#        if self.nparents > 1:
#            crosspopints = random.sample(reppop,int(np.floor(self.crossover_rate*len(reppop))))
