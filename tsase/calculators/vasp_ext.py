...
bool_keys = [
    ...
    'lwave',      #
    'lvdw',      #van der Waals interactions
]
...
class Vasp(Calculator):
    ...
    def run(self):
        ...
        if 'VASP_COMMAND' in os.environ:
            ...
            locals={}
            exec(compile(open(vasp, "rb").read(), vasp, 'exec'), {}, locals)
                ...
                if key == 'lreal':
                    if type(val) == str:
                       ...
                       if val:
                           ...
                           incar.write('.FALSE.\n')
        for key, val in list(self.dict_params.items()):
            if val is not None:
                if key == 'ldau_luj':
                    llist = ulist = jlist = ''
        ...
        for line in open('OSZICAR', 'r'):
            ...
            if line.lower().startswith('   1 f='):
                if all:
                    ...
                    energy_zero.append(float(line.split()[4]))
                else:
                    #energy_zero = float(line.split()[-1])
                    energy_zero = float(line.split()[4])
