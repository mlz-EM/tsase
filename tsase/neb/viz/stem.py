...
def _torus_distance_matrix(actual_positions, ideal_positions, cell_xy):
    """Return exact pair distances under projected periodic boundaries."""
    ...
    actual_xy = _frac_to_xy(actual_positions, cell_xy)
    cost = np.full((len(actual_positions), len(ideal_positions)), np.inf, dtype=float)
    for shift in _IMAGE_SHIFTS:
        shifted_ideal_xy = _frac_to_xy(ideal_positions - shift, cell_xy)
        np.minimum(cost, cdist(actual_xy, shifted_ideal_xy), out=cost)
    return cost


def _periodic_image_index(projected_points, cell_xy):
    """Build a Cartesian KD-tree containing neighboring periodic images."""

    projected_points = _wrap01(projected_points)
    image_positions = (projected_points[:, None, :] + _IMAGE_SHIFTS[None, :, :]).reshape(-1, 2)
    image_sources = np.repeat(np.arange(len(projected_points), dtype=int), len(_IMAGE_SHIFTS))
    image_xy = _frac_to_xy(image_positions, cell_xy)
    return cKDTree(image_xy), image_sources, image_xy


def _as_query_rows(values, row_count):
    """Normalize scalar/single-row ``cKDTree.query`` results to 2-D rows."""

    values = np.asarray(values)
    if values.ndim == 0:
        return values.reshape(1, 1)
    if values.ndim == 1:
        return values.reshape(row_count, -1)
    return values
...
def _connected_components(projected_fractional, cell_xy, cutoff_angstrom):
    ...
    tree, image_sources, _ = _periodic_image_index(projected_fractional, cell_xy)
    for image_a, image_b in tree.query_pairs(float(cutoff_angstrom)):
        index_a = int(image_sources[int(image_a)])
        index_b = int(image_sources[int(image_b)])
        if index_a != index_b:
...
def _nearest_component_distances(column_components, cell_xy, limit=12):
    if len(column_components) < 2 or limit <= 0:
        return []

    projected = np.asarray([component.projected_fractional for component in column_components], dtype=float)
    tree, image_sources, image_xy = _periodic_image_index(projected, cell_xy)
    query_k = min(len(image_xy), len(_IMAGE_SHIFTS) * (int(limit) + 1))
    distances, image_indices = tree.query(_frac_to_xy(projected, cell_xy), k=query_k)
    distances = _as_query_rows(distances, len(projected))
    image_indices = _as_query_rows(image_indices, len(projected))

    pair_distances = {}
    for left_index in range(len(column_components)):
        for distance, image_index in zip(distances[left_index], image_indices[left_index]):
            if not np.isfinite(distance) or image_index >= len(image_sources):
                continue
            right_index = int(image_sources[int(image_index)])
            if right_index == left_index:
                continue
            pair_key = tuple(sorted((left_index, right_index)))
            current = pair_distances.get(pair_key)
            if current is None or distance < current:
                pair_distances[pair_key] = float(distance)

    closest = sorted(pair_distances.items(), key=lambda item: item[1])[:limit]
    pairs = []
    for (left_index, right_index), distance in closest:
        left = column_components[left_index]
        right = column_components[right_index]
        pairs.append(
            {
...
def _nearest_periodic_pair(projected_points, cell_xy):
    projected_points = _wrap01(projected_points)
    if len(projected_points) < 2:
        return None, None

    tree, image_sources, image_xy = _periodic_image_index(projected_points, cell_xy)
    query_k = min(len(image_xy), len(_IMAGE_SHIFTS) + 1)
    distances, image_indices = tree.query(_frac_to_xy(projected_points, cell_xy), k=query_k)
    distances = _as_query_rows(distances, len(projected_points))
    image_indices = _as_query_rows(image_indices, len(projected_points))

    best_pair = None
    best_distance = None
    for left_index in range(len(projected_points)):
        for distance, image_index in zip(distances[left_index], image_indices[left_index]):
            if not np.isfinite(distance) or image_index >= len(image_sources):
                continue
            right_index = int(image_sources[int(image_index)])
            ...
            if best_distance is None or distance < best_distance:
                ...
                best_pair = tuple(sorted((left_index, right_index)))
            break
    return best_pair, best_distance


def _merge_disjoint_close_pairs(columns, weights, cell_xy, merge_threshold, merge_count):
    if merge_count <= 0 or len(columns) < 2:
        return columns, weights, []

    projected_points = _wrap01(columns)
    tree, image_sources, image_xy = _periodic_image_index(projected_points, cell_xy)

    pair_distances = {}
    for image_a, image_b in tree.query_pairs(float(merge_threshold)):
        source_a = int(image_sources[int(image_a)])
        source_b = int(image_sources[int(image_b)])
        if source_a == source_b:
            continue
        pair = tuple(sorted((source_a, source_b)))
        distance = float(np.linalg.norm(image_xy[int(image_a)] - image_xy[int(image_b)]))
        current = pair_distances.get(pair)
        if current is None or distance < current:
    ...
    merged_weights = []
    merges = []
    for (i, j), distance in sorted(pair_distances.items(), key=lambda item: item[1]):
        ...
        merged_weights.append(weights[i] + weights[j])
        active[i] = False
        active[j] = False
        merges.append(
            {
    ...
    next_columns = [columns[index] for index in range(len(columns)) if active[index]]
    next_weights = [weights[index] for index in range(len(weights)) if active[index]]
    next_columns.extend(merged_columns)
    next_weights.extend(merged_weights)
    return next_columns, next_weights, merges
...
def _repair_oversegmented_columns(columns, weights, cell_xy, target_count, label, grid_size):
    ...
    merges = []
    columns, weights, initial_merges = _merge_disjoint_close_pairs(
        columns,
        weights,
        cell_xy,
        merge_threshold,
        len(columns) - target_count,
    )
    merges.extend(initial_merges)

    while len(columns) > target_count:
        best_pair, best_distance = _nearest_periodic_pair(columns, cell_xy)
...
def _assignment_cost(actual_positions, ideal_positions, cell_xy):
    return _torus_distance_matrix(actual_positions, ideal_positions, cell_xy)


def _assign_positions_to_ideal(actual_positions, ideal_positions, cell_xy):
    actual_positions = np.asarray(actual_positions, dtype=float)
    ideal_positions = np.asarray(ideal_positions, dtype=float)
    if len(actual_positions) < len(ideal_positions):
        raise ValueError("fewer actual positions than ideal sites")

    cost = _assignment_cost(actual_positions, ideal_positions, cell_xy)
    rows, cols = linear_sum_assignment(cost)
    assigned_actual = np.full(len(ideal_positions), -1, dtype=int)
    assigned_distances = np.full(len(ideal_positions), np.inf, dtype=float)
    for row, col in zip(rows, cols):
        assigned_actual[int(col)] = int(row)
        assigned_distances[int(col)] = float(cost[row, col])
    return assigned_actual, assigned_distances
...
def _build_pb_grid(pb_columns, cell_xy, *, grid_size, previous_pb_plot_grid=None):
    ...
    for u_levels in u_candidates:
        for v_levels in v_candidates:
            ...
            assigned_actual, assigned_distances = _assign_positions_to_ideal(pb_columns, ideal_nodes, cell_xy)

            assigned = np.zeros((grid_size, grid_size, 2), dtype=float)
            assignment_cost = float(np.sum(assigned_distances))
            for col, row in enumerate(assigned_actual):
                if row < 0:
                    continue
...
def _assign_to_grid(actual_positions, ideal_grid, cell_xy, label, max_reasonable_distance):
    ...
    assigned_actual, assigned_distances = _assign_positions_to_ideal(actual_positions, flat_ideal, cell_xy)
    assigned = np.zeros_like(ideal_grid)
    for col, row in enumerate(assigned_actual):
        if row < 0:
            continue
        assigned.reshape(-1, 2)[col] = actual_positions[row]
        if assigned_distances[col] > max_reasonable_distance:
            raise StemAnalysisError(
                f"a {label} column was assigned too far from its ideal topology site",
                diagnostics={
                    "label": label,
                    "max_reasonable_distance_angstrom": max_reasonable_distance,
                    "assigned_distance_angstrom": float(assigned_distances[col]),
...
def _assign_edge_oxygen_columns(actual_positions, ideal_grid, cell_xy, max_reasonable_distance):
    ...
    for col, row in enumerate(assigned_actual):
        ...
        selected_costs.append(float(assigned_distances[col]))
        if assigned_distances[col] > max_reasonable_distance:
            raise StemAnalysisError(
    ...
    remaining_costs = _torus_distance_matrix(actual_positions[remaining_rows], flat_ideal, cell_xy)
    remaining_best_costs = [float(value) for value in np.min(remaining_costs, axis=1)]
    return assigned, {
