...
def _relative_config_path(path, config_dir):
    """Express a runtime path relative to a generated config directory."""

    return Path(relpath(Path(path).resolve(), Path(config_dir).resolve())).as_posix()
...
def preprocess_field_ssneb_control_points(config_path, *, output_dir=None):
    ...
    derived_config.setdefault("path", {}).setdefault("source", {})["files"] = [
        _relative_config_path(path, destination_dir) for path in processed_extxyz_paths
    ]
    derived_config["path"]["source"]["kind"] = "control_points"
    derived_config["path"]["source"]["indices"] = list(config.structure_indices)
    derived_model = derived_config.setdefault("model", {})
    if config.calculator_mode.kind == "mace_field":
        derived_model.pop("charges", None)
    calculator_config = derived_model.setdefault("calculator", {})
    model_path = calculator_config.get("model_path")
    if model_path is not None:
        source_model_path = Path(model_path).expanduser()
        if not source_model_path.is_absolute():
            source_model_path = config_path.parent / source_model_path
        calculator_config["model_path"] = _relative_config_path(source_model_path, destination_dir)
    derived_model["reference"] = {
        "kind": "file",
        "file": _relative_config_path(polarization_reference_extxyz_path, destination_dir),
    }
    derived_config.setdefault("run", {})["root"] = _relative_config_path(config.run_dir, destination_dir)
    derived_config.pop("preprocess", None)
