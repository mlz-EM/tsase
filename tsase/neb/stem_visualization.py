...
def _quiver_geometry(vectors_xy, constant_length_xy):
    display = np.zeros_like(vectors_xy)
    magnitudes = np.linalg.norm(vectors_xy, axis=-1)
    mask = magnitudes > 1e-15
    display[mask] = (vectors_xy[mask] / magnitudes[mask, None]) * constant_length_xy
    return display
...
def render_projected_frame(analysis, output_path):
    ...
    a_length = float(np.linalg.norm(a_axis))
