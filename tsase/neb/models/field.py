...
def resolve_field_config(cell, field_config=None):
    """Resolve a field configuration to one Cartesian vector.

    Direction-based input separates orientation from magnitude and supports
    either Cartesian axes or crystallographic lattice-vector coordinates.
    """

    config = dict(field_config or {})
    kind = str(config.get("kind", "none")).lower()
    if kind == "none":
        return resolve_field_vector(cell)
    if kind == "cartesian":
        return resolve_field_vector(cell, field=config.get("value"))
    if kind == "crystal":
        return resolve_field_vector(cell, field_crystal=config.get("value"))
    if kind != "direction":
        raise ValueError("model.field.kind must be one of: none, cartesian, crystal, direction")

    direction = np.asarray(config.get("direction"), dtype=float)
    if direction.shape != (3,):
        raise ValueError("model.field.direction must contain exactly 3 components")

    basis = str(config.get("basis", "cartesian")).lower()
    if basis == "crystal":
        direction = crystal_field_to_cartesian(cell, direction)
    elif basis != "cartesian":
        raise ValueError("model.field.basis must be one of: cartesian, crystal")

    norm = float(np.linalg.norm(direction))
    if norm <= 0.0:
        raise ValueError("model.field.direction must be non-zero")
    magnitude = float(config.get("magnitude"))
    if magnitude < 0.0:
        raise ValueError("model.field.magnitude must be non-negative")
