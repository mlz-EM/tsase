...
### User-provided contract

For a MACE-field ASE calculator, the maintained runtime must use exactly this contract:
...
Interpretation:
- `E` is already the field-adjusted enthalpy-like energy to be used along the path,
- `P` is the polarization to record and use for output/plotting,
- no polarization reference structure is needed for this mode,
- no external enthalpy wrapper should be applied.
...
- calculator energy is already the enthalpy-adjusted quantity to use as the path energy,
- atomic forces returned by the calculator are already the correct forces for that enthalpy model,
- polarization is obtained directly from `atoms.calc.results["polarization"]`,
- no external polarization reference atoms are used,
- no extra field-force correction is added externally,
- no second enthalpy wrapper is applied.

### Recording rule for `mace_field`

For `mace_field`, only the following physics quantities should be recorded as maintained energy/polarization outputs:
- enthalpy-adjusted energy
- polarization

That means:
- do **not** record or infer an intrinsic/base energy unless the calculator explicitly exposes it and the user later requests that extension,
- do **not** record or infer a separate field-energy decomposition for the maintained `mace_field` mode,
- do **not** reconstruct intrinsic energy from polarization or field values.

The maintained `mace_field` path should therefore treat the calculator as providing a total field-adjusted energy and polarization only.
...
- polarization reference handling remains available where required
...
- do not use or require reference polarization atoms
- record only enthalpy-adjusted energy plus polarization outputs in the maintained path
...
The maintained YAML should accept explicit entries to plot/export.
...
For wrapped intrinsic-field calculators, the maintained runtime may support entries such as:
- `intrinsic_energy`
- `enthalpy_adjusted`
- `field_energy`
- `polarization_mag`
...
For `mace_field`, the maintained runtime should support only the entries that are actually defined by the contract:
- `enthalpy_adjusted`
- `polarization_mag`
- `polarization_x`
- `polarization_y`
...
It should **not** plot or export `intrinsic_energy` or `field_energy` for `mace_field` unless a later, explicitly supported calculator contract provides them.
...
- energy used along the path equals `atoms.get_potential_energy()` from the calculator,
- forces are taken directly from the calculator and not modified by an extra field term,
- polarization for plots/diagnostics is pulled directly from `atoms.calc.results["polarization"]`,
- no reference polarization structure is required,
...
- no fake intrinsic or field-energy decomposition is emitted for `mace_field`,
...
- direct energy consumption from `atoms.get_potential_energy()`,
- direct polarization retrieval from `atoms.calc.results["polarization"]`,
- no reference polarization handling,
...
12. `mace_field` energies are consumed directly from `atoms.get_potential_energy()` as enthalpy-adjusted totals without double counting
13. `mace_field` forces are taken directly from the calculator without extra field-force corruption
14. `mace_field` polarization is retrieved directly from `results["polarization"]`
15. `mace_field` does not require or use reference polarization atoms
16. plots/CSV for `mace_field` emit only enthalpy-adjusted energy and polarization-derived quantities in the maintained path

### E. Regression-preservation tests

17. wrapped intrinsic-calculator field path remains numerically consistent with the preserved baseline
18. FIRE and SSNEB force/tangent behavior remain intact for unchanged modes
19. final maintained example scripts run successfully
...
- [ ] `mace_field` energy is consumed directly from `atoms.get_potential_energy()`
- [ ] `mace_field` polarization is taken directly from `atoms.calc.results["polarization"]`
- [ ] `mace_field` does not require a reference polarization structure
- [ ] energy/profile plotting is driven by explicit YAML entries
- [ ] for `mace_field`, the maintained outputs only record enthalpy-adjusted energy and polarization-derived quantities
