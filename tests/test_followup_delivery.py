    ...
    def test_mace_field_mode_uses_direct_contract_and_filtered_outputs(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("tsase.neb.workflows.config.load_mace_calculator", return_value=DummyMaceFieldCalculator):
                ...
                self.assertIsNone(config.charge_map)
                self.assertNotIn("charges", config.resolved_config["model"])
                self.assertEqual(
                ...
                self.assertTrue(
                    all("field_charges" not in image.arrays for image in result["band"].path)
                )
