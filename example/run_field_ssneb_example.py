...
FIELD = None
FIELD_CRYSTAL = np.array([0.00, 0.000, 0.00], dtype=float)
...
METHOD = "ci"
CI_ACTIVATION_FORCE = 0.001
FMAX = 0.05
MAX_STEPS = 2000
OUTPUT_INTERVAL = 50
PLOT_PROPERTY = "P_mag"
IMAGE_UPDATE_FACTORS = None  # {16: 0.5, 17: 1.5}
IMAGE_UPDATE_ITERATIONS = 50
...
def main():
    ...
    copied_structure_records = artifacts.copy_inputs(
        {
            ...
            for index, path in zip(STRUCTURE_INDICES, STRUCTURE_PATHS)
        }
    )
    ...
    for atoms in structures:
        ...
        optimizer = BFGS(atoms)  # you can also use BFGS(atoms)
        optimizer.run(fmax=0.02)  # fmax in eV/Å
    ...
    polarization_reference_path = artifacts.run_dir / "polarization_reference.cif"
    ...
    if RESTART_FROM_XYZ is not None:
        restart_record = artifacts.copy_inputs({"restart": RESTART_FROM_XYZ})[0]
        neb.load_band_configuration_from_xyz(band, restart_record["copied_to"])
